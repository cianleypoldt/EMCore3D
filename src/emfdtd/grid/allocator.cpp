...
bool YeeGrid::grid_allocate() {
    if (Emfdtd::total_heap_allocation + cell_count * memory_per_cell > em_const::memory_cap) {
    ...
    Emfdtd::total_heap_allocation += cell_count * memory_per_cell;
    ...
    spdlog::debug("Grid allocated {} bytes ({:.2f} MB). Total memory in use is {} bytes ({:.2f} MB)",
                  ...
                  Emfdtd::total_heap_allocation,
                  Emfdtd::total_heap_allocation / (1024.0 * 1024.0));
